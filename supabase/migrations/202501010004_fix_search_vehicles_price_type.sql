/*
  # Fix price data type mismatch in search_vehicles
  This migration resolves the `42804` error ("structure of query does not match function result type") by aligning the data type of the returned `price` column with the function's signature.

  1.  **Type Casting**: The `price` column in the underlying `vehicle_listings_detailed` view is of type `NUMERIC(10,2)`, but the `search_vehicles` function expects to return an `INT`. This migration applies an explicit cast (`pl.price::INT`) in the final `SELECT` statement. This converts the numeric value to an integer, satisfying the function's return type definition and resolving the error.
*/
